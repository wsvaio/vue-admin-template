...
// 生成主题色
export const generateColor = <T extends string>(
	colors: [T, string][],
	darkTheme = false,
	darkThemeMixColor = "#141414"
) => {
	const actions = [
		...
		["Hover", (color: string) => getColorPalette(color, 5, darkTheme, darkThemeMixColor)],
		["Pressed", (color: string) => getColorPalette(color, 7, darkTheme, darkThemeMixColor)],
		["Active", (color: string) => colord(color).alpha(0.1).toHex()],

		["1", (color: string) => getColorPalette(color, 1, darkTheme, darkThemeMixColor)],
		["2", (color: string) => getColorPalette(color, 2, darkTheme, darkThemeMixColor)],
		["3", (color: string) => getColorPalette(color, 3, darkTheme, darkThemeMixColor)],
		["4", (color: string) => getColorPalette(color, 4, darkTheme, darkThemeMixColor)],
		["5", (color: string) => getColorPalette(color, 5, darkTheme, darkThemeMixColor)],
		["6", (color: string) => getColorPalette(color, 6, darkTheme, darkThemeMixColor)],
		["7", (color: string) => getColorPalette(color, 7, darkTheme, darkThemeMixColor)],
		["8", (color: string) => getColorPalette(color, 8, darkTheme, darkThemeMixColor)],
		["9", (color: string) => getColorPalette(color, 9, darkTheme, darkThemeMixColor)],
		["10", (color: string) => getColorPalette(color, 10, darkTheme, darkThemeMixColor)],
	] as const;
	type Scene =
		| ""
		| "Suppl"
